...
const decimalAdjust = (type, inputNumber, exp) => {
  ...
  exp = +exp;
  let checkForNaN =
    isNaN(inputNumber) || !(typeof exp === 'number' && exp % 1 === 0);
  if (checkForNaN) {
    return NaN;
  }
  inputNumber = inputNumber.toString().split('e');
  inputNumber = Math[type](
    +(inputNumber[0] + 'e' + (inputNumber[1] ? +inputNumber[1] - exp : -exp))
  );
  inputNumber = inputNumber.toString().split('e');
  return +(
    inputNumber[0] +
    'e' +
    (inputNumber[1] ? +inputNumber[1] + exp : exp)
  );
};
...
export const getStepClass = (step, activeStep) =>
  step === activeStep
    ? 'step-navigation step-navigation_active'
    : 'step-navigation';
...
export const validateSupply = supply =>
  isNaN(Number(supply)) === false && Number(supply) > 0;
...
export const validateRate = rate =>
  isNaN(Number(rate)) === false && Number(rate) > 0;
