...
export function getWhiteListWithCapCrowdsaleAssets() {
  ...
  const crowdsaleFiles = ['sol', ...binAbi].map(
    ext => `${contractsRoute}${crowdsaleFilename}_flat.${ext}`
  );
  const tokenFiles = binAbi.map(
    ext => `${contractsRoute}${crowdsaleFilename}Token_flat.${ext}`
  );
  const pricingFiles = binAbi.map(
    ext => `${contractsRoute}${crowdsaleFilename}PricingStrategy_flat.${ext}`
  );
  const finalizeFiles = binAbi.map(
    ext => `${contractsRoute}FinalizeAgent_flat.${ext}`
  );
  const nullFiles = binAbi.map(
    ext => `${contractsRoute}NullFinalizeAgent_flat.${ext}`
  );
  const registryFiles = binAbi.map(
    ext => `${contractsRoute}Registry_flat.${ext}`
  );
  const states = crowdsaleFiles
    .concat(tokenFiles, pricingFiles, finalizeFiles, nullFiles, registryFiles)
    .map(setFlatFileContentToState);

  return Promise.all(states).then(state => {
...
export const getconstructorParams = (
  abiConstructor,
  vals,
  crowdsaleNum,
  isCrowdsale
    ...
    if (vals.length > 0) {
      ...
      switch (inp.name) {
        case '_start':
          params.vals.push(
            toFixed(
              new Date(tierStore.tiers[crowdsaleNum].startTime).getTime() / 1000
            ).toString()
          );
          break;
        case '_end':
          params.vals.push(
            toFixed(
              new Date(tierStore.tiers[crowdsaleNum].endTime).getTime() / 1000
            ).toString()
          );
          break;
        case '_rate':
        ...
        case '_globalMinCap':
          params.vals.push(
            tierStore.tiers[0].whitelistEnabled !== 'yes'
              ? tokenStore.globalmincap
                ? toFixed(
                    tokenStore.globalmincap * 10 ** tokenStore.decimals
                  ).toString()
                : 0
              : 0
          );
        ...
        case '_maximumSellableTokens':
          params.vals.push(
            toFixed(
              tierStore.tiers[crowdsaleNum].supply * 10 ** tokenStore.decimals
            ).toString()
          );
          break;
        ...
        case '_mintable':
          ...
          break;
        case '_oneTokenInWei':
          let oneTokenInETHRaw = toFixed(
            1 / tierStore.tiers[crowdsaleNum].rate
          ).toString();
          let oneTokenInETH = floorToDecimals(
        ...
        case '_isUpdatable':
          params.vals.push(
            tierStore.tiers[crowdsaleNum].updatable
              ? tierStore.tiers[crowdsaleNum].updatable === 'on' ? true : false
              : false
          );
          break;
        case '_isWhiteListed':
          params.vals.push(
            tierStore.tiers[0].whitelistEnabled
              ? tierStore.tiers[0].whitelistEnabled === 'yes'
              : false
          );
          break;
