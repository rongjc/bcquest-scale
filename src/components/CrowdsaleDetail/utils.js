...
export const updateTierAttribute = (attribute, value, addresses) => {
  ...
  if (
    ...
    if (attribute === 'startTime' || attribute === 'endTime') {
      ...
      value = [
        toFixed(
          parseInt(value, 10) * 10 ** parseInt(tokenStore.decimals, 10)
        ).toString()
      ];
    } else {
      // whitelist
      value = value.reduce(
        (toAdd, whitelist) => {
          ...
          toAdd[1].push(true);
          toAdd[2].push(
            whitelist.min * 10 ** decimals
              ? toFixed((whitelist.min * 10 ** decimals).toString())
              : 0
          );
          toAdd[3].push(
            whitelist.max * 10 ** decimals
              ? toFixed((whitelist.max * 10 ** decimals).toString())
              : 0
          );
          return toAdd;
        },
  ...
  if (attribute === 'whitelist') {
    ...
    const currentTierIndex = crowdsaleStore.selected.initialTiersValues.findIndex(
      tier => tier.addresses.crowdsaleAddress === addresses.crowdsaleAddress
    );
  ...
  return contractAddresses.reduce((promise, contractAddress) => {
    return promise.then(() => {
      return attachToContract(abi, contractAddress).then(contract => {
        const method = contract.methods[methods[attribute]];

        return method(...value)
          .estimateGas()
          .then(estimatedGas => {
            return sendTXToContract(
              method(...value).send({
...
const extractWhitelistInformation = (isWhitelisted, crowdsaleMethods) => {
  ...
  if (isWhitelisted && crowdsaleMethods.whitelistedParticipantsLength) {
    return crowdsaleMethods
      .whitelistedParticipantsLength()
      .call()
      .then(participantsCount => {
        for (
          let participantIndex = 0;
          participantIndex < participantsCount;
          participantIndex++
        ) {
          whenWhitelistedAddresses.push(
            crowdsaleMethods.whitelistedParticipants(participantIndex).call()
          );
        }
      ...
      .then(whitelistedAddresses => {
        const whenAccountData = whitelistedAddresses.map(address =>
          crowdsaleMethods.earlyParticipantWhitelist(address).call()
        );

        return Promise.all(whenAccountData).then(accountData => [
          isWhitelisted,
  ...
  return Promise.resolve([
    isWhitelisted,
    whenWhitelistedAddresses,
    whitelistedAccounts
  ]);
};

const crowdsaleData = crowdsaleAddress => {
  return attachToContract(contractStore.crowdsale.abi, crowdsaleAddress).then(
    crowdsaleContract => {
      ...
      return whenIsWhitelisted
        .then(isWhitelisted =>
          extractWhitelistInformation(isWhitelisted, methods)
        )
...
const tokenData = tokenAddress => {
  return attachToContract(contractStore.token.abi, tokenAddress).then(
    tokenContract => {
...
const pricingStrategyData = pricingStrategyAddress => {
  return attachToContract(
    contractStore.pricingStrategy.abi,
    pricingStrategyAddress
  ).then(pricingStrategyContract =>
    pricingStrategyContract.methods.oneTokenInWei().call()
  );
};
    ...
    crowdsaleContract => {
      ...
      return methods
        ...
        .then(joinedCrowdsalesLen => {
          ...
          for (
            let joinedTierIndex = 0;
            joinedTierIndex < joinedCrowdsalesLen;
            joinedTierIndex++
          ) {
            joinedCrowdsales.push(
              methods.joinedCrowdsales(joinedTierIndex).call()
            );
          }
...
export const processTier = (crowdsaleAddress, crowdsaleNum) => {
  ...
  return crowdsaleData(crowdsaleAddress)
    .then(
      ([
        ...
        crowdsaleStore.setSelectedProperty(
          'updatable',
          crowdsaleStore.selected.updatable || updatable
        );
    ...
    .then(
      ([
        ...
        const maxCapBeforeDecimals =
          parseInt(toFixed(maximumSellableTokens), 10) / 10 ** tokenDecimals;
        const tierCap = maxCapBeforeDecimals
          ? maxCapBeforeDecimals.toString()
          : 0;
        const standardCrowdsaleSupply = !isNaN(crowdsaleStore.supply)
          ? crowdsaleStore.supply.toString()
          : 0;

        newTier.supply =
          contractStore.contractType === WHITELIST_WITH_CAP
            ? tierCap
    ...
    .then(([whitelistAccounts, rate]) => {
      ...
      const tokensPerETHTiers = !isNaN(1 / rate)
        ? 1 / web3.utils.fromWei(toFixed(rate).toString(), 'ether')
        : 0;

      newTier.rate =
        contractStore.contractType === WHITELIST_WITH_CAP
          ? tokensPerETHTiers
          : tokensPerETHStandard;
