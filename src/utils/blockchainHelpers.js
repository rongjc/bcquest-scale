        ...
        checkTxMined(_txHash, function pollingReceiptCheck(err, receipt) {
          ...
          if (receipt) {
            if (receipt.blockNumber) {
              console.log(
                `${typeDisplayName} ${txHash} is mined from polling of tx receipt`
              );
              isMined = true;
          ...
          function repeatPolling() {
            console.log(
              `${typeDisplayName} ${txHash} is still pending. Polling of transaction once more`
            );
...
export function getRegistryAddress() {
  ...
  return web3.eth.net.getId().then(networkId => {
    const registryAddressMap = JSON.parse(
...
function getRegistryContract() {
  ...
  return Promise.all([whenRegistryAbi, whenRegistryAddress]).then(
    ([abi, address]) => attachToContract(abi, address)
  );
}
      ...
      return registry.methods
        ...
        .then(count => {
          ...
          for (let i = 0; i < +count; i++) {
            crowdsales.push(
              registry.methods.deployedContracts(account, i).call()
            );
          }
