  ...
  attachToContract(abi, addr)
    .then(crowdsaleContract => {
      ...
      crowdsaleContract.methods
        .joinedCrowdsalesLen()
        .call(function(err, joinedCrowdsalesLen) {
          if (err) {
            console.log(err);
            return cb([]);
          }

          getJoinedTiersRecursively(
            0,
            crowdsaleContract,
            joinedCrowdsales,
            joinedCrowdsalesLen,
            function(_joinedCrowdsales) {
...
function getJoinedTiersRecursively(
  i,
  crowdsaleContract,
  joinedCrowdsales,
  joinedCrowdsalesLen,
  ...
  crowdsaleContract.methods
    .joinedCrowdsales(i)
    .call(function(err, joinedCrowdsale) {
...
export function findCurrentContractRecursively(i, firstCrowdsaleContract, cb) {
  ...
  if (i === contractStore.crowdsale.addr.length)
    return cb(firstCrowdsaleContract, i);
      ...
      .then(crowdsaleContract => {
        ...
        if (crowdsaleContract.methods.startBlock) {
          ...
          crowdsaleContract.methods.startBlock().call((err, startBlock) => {
            ...
            if (
              !crowdsalePageStore.startBlock ||
              crowdsalePageStore.startBlock > startBlock
            )
        ...
        if (crowdsaleContract.methods.endBlock) {
          ...
          crowdsaleContract.methods.endBlock().call((err, endBlock) => {
            ...
            web3.eth.getBlockNumber((err, curBlock) => {
              ...
              let blocksDiff =
                parseInt($this.crowdsalePageStore.endBlock, 10) -
                parseInt(curBlock, 10);
...
export function getAccumulativeCrowdsaleData() {
  ...
  let promises = contractStore.crowdsale.addr.map(crowdsaleAddr => {
    return attachToContract(contractStore.crowdsale.abi, crowdsaleAddr).then(
      crowdsaleContract => {
        // eslint-disable-line no-loop-func
        if (!crowdsaleContract) return noContractAlert();

        let getWeiRaised = crowdsaleContract.methods
          .weiRaised()
          .call()
          .then(weiRaised => {
            let newWeiRaised;
            if (crowdsalePageStore.weiRaised) {
              newWeiRaised =
                crowdsalePageStore.weiRaised + parseInt(weiRaised, 10);
            } else {
            ...
            crowdsalePageStore.setProperty(
              ...
              parseFloat(
                web3.utils.fromWei(
                  toFixed(crowdsalePageStore.weiRaised).toString(),
                  'ether'
                )
              )
            );
        ...
        let getTokensSold = crowdsaleContract.methods
          .tokensSold()
          .call()
          .then(tokensSold => {
            if (crowdsalePageStore.tokensSold) {
              crowdsalePageStore.setProperty(
                'tokensSold',
                crowdsalePageStore.tokensSold + parseInt(tokensSold, 10)
              );
            } else {
              crowdsalePageStore.setProperty(
                'tokensSold',
                parseInt(tokensSold, 10)
              );
            }
          });

        let getMaximumSellableTokens = crowdsaleContract.methods
          .maximumSellableTokens()
          .call()
          .then(maximumSellableTokens => {
            const maxSellableTokens = crowdsalePageStore.maximumSellableTokens;
            if (maxSellableTokens) {
              crowdsalePageStore.setProperty(
                'maximumSellableTokens',
                maxSellableTokens + parseInt(toFixed(maximumSellableTokens), 10)
              );
            } else {
              crowdsalePageStore.setProperty(
                'maximumSellableTokens',
                parseInt(toFixed(maximumSellableTokens), 10)
              );
            }

            //calc maximumSellableTokens in Eth
            return setMaximumSellableTokensInEth(
              crowdsaleContract,
              maximumSellableTokens
            );
          });

        let getInvestors = crowdsaleContract.methods
          .investorCount()
          .call()
          .then(investors => {
...
function setMaximumSellableTokensInEth(
  crowdsaleContract,
  maximumSellableTokens
) {
  return crowdsaleContract.methods
    .pricingStrategy()
    .call()
    ...
    .then(pricingStrategyContract => {
      ...
      return pricingStrategyContract.methods
        .oneTokenInWei()
        .call()
        .then(oneTokenInWei => {
          if (crowdsalePageStore.maximumSellableTokensInWei) {
            crowdsalePageStore.setProperty(
              ...
              crowdsalePageStore.maximumSellableTokensInWei +
                parseInt(oneTokenInWei, 10) *
                  maximumSellableTokens /
                  10 ** tokenStore.decimals
            );
          } else {
            crowdsalePageStore.setProperty(
              'maximumSellableTokensInWei',
            ...
            tokenContract.methods.balanceOf.call(
              ...
              (err, balanceOf) => {
                ...
                const tokenAmountOf = crowdsalePageStore.tokenAmountOf
                  ? crowdsalePageStore.tokenAmountOf
                  : 0;
                crowdsalePageStore.setProperty(
                  'tokenAmountOf',
                  tokenAmountOf + parseInt(balanceOf, 10)
                );
...
export function getPricingStrategyData() {
  return new Promise((resolve, reject) => {
    attachToContract(
      contractStore.pricingStrategy.abi,
      contractStore.pricingStrategy.addr
    )
      .then(pricingStrategyContract => {
