...
export function noMetaMaskAlert() {
  sweetAlert2({
    title: 'Warning',
    html:
      "You don't have Metamask installed or login. Please install metamask and login.",
    type: 'warning'
  });
}
...
export function noContractDataAlert() {
  sweetAlert2({
    title: 'Warning',
    html:
      'The crowdsale data is empty. There is nothing to deploy. Please, start Token Wizard from the beginning.',
    type: 'warning'
  });
}
...
export function invalidCrowdsaleAddrAlert() {
  sweetAlert2({
    title: 'Warning',
    html:
      'Invalid crowdsale address is indicated in config and/or in query string.',
    type: 'warning'
  });
}
...
export function investmentDisabledAlert(startBlock, curBlock) {
  sweetAlert2({
    title: 'Warning',
    html:
      "Wait, please. Crowdsale company hasn't started yet. It'll start from <b>" +
      startBlock +
      '</b> block. Current block is <b>' +
      curBlock +
      '</b>.',
    type: 'warning'
  });
...
export function incorrectNetworkAlert(
  correctNetworkName,
...
export function noDeploymentOnMainnetAlert() {
  sweetAlert2({
    title: 'Warning',
    html:
      "Wizard is under maintenance on Ethereum Mainnet. Please come back later or use Kovan/Rinkeby/POA. Follow <a href='https://twitter.com/poanetwork'>https://twitter.com/poanetwork</a> for status.",
    type: 'warning'
  });
}

export function warningOnMainnetAlert(
  tiersCount,
  priceSelected,
  reservedCount,
  whitelistCount,
  ...
  let gasRequired =
    GAS_REQUIRED.DEFAULT +
    GAS_REQUIRED.WHITELIST * whitelistCount +
    GAS_REQUIRED.RESERVED_TOKEN * reservedCount;
  ...
  sweetAlert2({
    ...
    html: `You are about to sign ${estimatedTxsCount} TXs. You will see an individual Metamask windows for each of it.
     ...
     crowdsale for you. The total cost will be around ${estimatedCost.toFixed(
       2
     )} ETH. Are you sure you want to
     proceed?`,
    type: 'warning',
...
export function noGasPriceAvailable() {
  sweetAlert2({
    ...
    html:
      "Token Wizard wasn't able to request current Gas Prices from the blockchain, custom values will be used",
    type: 'warning'
  });
}
...
export function notTheOwner() {
  sweetAlert2({
    title: 'Not The Owner',
    html:
      "Current user is not the owner of the Crowdsale, thus you won't be able to modify it",
    type: 'warning'
  });
}
...
export function cancellingIncompleteDeploy() {
  return sweetAlert2({
    ...
    html:
      'Are you sure you want to cancel the deployment of the crowdsale? This action cannot be undone.',
    type: 'warning',
    showCancelButton: true,
    cancelButtonText: 'No',
    confirmButtonText: 'Yes',
