...
export const setupContractDeployment = () => {
  ...
  const whenTokenABIConstructor = Promise.resolve(tokenAddr).then(tokenAddr => {
    if (!tokenAddr) {
      return getEncodedABIClientSide(tokenABI, [], 0).then(ABIEncoded => {
  ...
  const whenPricingStrategyContract = tierStore.tiers.map((value, index) => {
    return getEncodedABIClientSide(pricingStrategyABI, [], index).then(
      ABIEncoded => {
        console.log(
          'pricingStrategy ABI Encoded params constructor:',
          ABIEncoded
        );
        const newContract = contractStore.pricingStrategy.abiConstructor.concat(
          ABIEncoded
        );
        contractStore.setContractProperty(
          'pricingStrategy',
...
export const deploySafeMathLibrary = () => {
  return [
    () => {
      ...
      return deployContract(abiSafeMathLib, binSafeMathLib, []).then(
        safeMathLibAddr => {
          contractStore.setContractProperty(
            'safeMathLib',
            'addr',
            ...
            Object.keys(contractStore)
              ...
              .forEach(key => {
                if (contractStore[key].bin) {
                  const strToReplace =
                    '__:SafeMathLibExt_______________________';
                  const newBin = replaceAll(
                    strToReplace,
                    safeMathLibAddr.substr(2),
                    contractStore[key].bin
                  );
...
const getTokenParams = token => {
  const whitelistWithGlobalMinCap =
    tierStore.tiers[0].whitelistEnabled !== 'yes' && tierStore.globalMinCap;
  const minCap = whitelistWithGlobalMinCap
    ? toFixed(tierStore.globalMinCap * 10 ** token.decimals).toString()
    : 0;
...
const getPricingStrategyParams = tier => {
  const oneTokenInETH = floorToDecimals(
    TRUNC_TO_DECIMALS.DECIMALS18,
    1 / tier.rate
  );

  return [web3Store.web3.utils.toWei(oneTokenInETH, 'ether')];
};
  ...
  return tierStore.tiers.map((tier, index) => {
    return () => {
      ...
      return deployContract(
        abiPricingStrategy,
        binPricingStrategy,
        paramsPricingStrategy
      )
        .then(pricingStrategyAddr =>
          contractStore.pricingStrategy.addr.concat(pricingStrategyAddr)
        )
        .then(newPricingStrategy =>
          contractStore.setContractProperty(
            'pricingStrategy',
            'addr',
            newPricingStrategy
        ...
        .then(ABIEncoded =>
          contractStore.crowdsale.abiConstructor.concat(ABIEncoded)
        )
        .then(newContract =>
          contractStore.setContractProperty(
            'crowdsale',
...
const getCrowdSaleParams = index => {
  ...
  const { updatable, supply, tier, startTime, endTime } = tierStore.tiers[
    index
  ];

  const formatDate = date =>
    toFixed(parseInt(Date.parse(date) / 1000, 10).toString());
  ...
  return [
    ...
    toFixed('0'),
    toFixed(
      parseInt(supply, 10) * 10 ** parseInt(tokenStore.decimals, 10)
    ).toString(),
    updatable === 'on',
    ...
    return () => {
      ...
      return getNetworkVersion()
        ...
        .then(() => {
          ...
          const paramsCrowdsale =
            contractStore.contractType === whitelistwithcap
        ...
        .then(crowdsaleAddr => {
          ...
          const newCrowdsaleAddr = contractStore.crowdsale.addr.concat(
            crowdsaleAddr
          );
          contractStore.setContractProperty(
            'crowdsale',
  ...
  return [
    () => {
      ...
      return Promise.all([whenRegistryAddress, whenAccount])
        ...
        .then(() =>
          deploymentStore.setAsSuccessful('registerCrowdsaleAddress')
        );
    }
  ];
}
...
const getNullFinalizeAgentParams = index => {
  return [contractStore.crowdsale.addr[index]];
};

const getFinalizeAgentParams = index => {
  return [contractStore.token.addr, contractStore.crowdsale.addr[index]];
};
  ...
  return tierStore.tiers.map((tier, index, tiers) => {
    return () => {
      ...
      return getEncodedABIClientSide(abi, [], index)
        .then(ABIEncoded => {
          console.log(
            'finalizeAgent ABI encoded params constructor:',
            ABIEncoded
          );

          const newAbi = contractStore.finalizeAgent.abiConstructor.concat(
            ABIEncoded
          );
          contractStore.setContractProperty(
            'finalizeAgent',
        ...
        .then(finalizeAgentAddr => {
          console.log(
            '***Deploy finalize agent contract***',
            finalizeAgentAddr
          );

          const newAddr = contractStore.finalizeAgent.addr.concat(
            finalizeAgentAddr
          );
        ...
        .then(crowdsaleContract => {
          ...
          for (let i = 0; i < whitelist.length; i++) {
            if (!whitelist[i].deleted) {
              ...
              statuses.push(true);
              minCaps.push(
                whitelist[i].min * 10 ** tokenStore.decimals
                  ? toFixed(
                      (whitelist[i].min * 10 ** tokenStore.decimals).toString()
                    )
                  : 0
              );
              maxCaps.push(
                whitelist[i].max * 10 ** tokenStore.decimals
                  ? toFixed(
                      (whitelist[i].max * 10 ** tokenStore.decimals).toString()
                    )
                  : 0
              );
            }
          ...
          const opts = { gasPrice: generalStore.gasPrice };
          const method = crowdsaleContract.methods.setEarlyParticipantWhitelistMultiple(
            addrs,
            statuses,
            minCaps,
            maxCaps
    ...
    return () => {
      ...
      return attachToContract(abi, joinedContractAddresses[index])
        .then(crowdsaleContract => {
          ...
          const opts = { gasPrice: generalStore.gasPrice };
          const method = crowdsaleContract.methods.updateJoinedCrowdsalesMultiple(
            joinedContractAddresses
          );
    ...
    return () => {
      ...
      return attachToContract(abi, crowdsaleAddr)
        .then(crowdsaleContract => {
          ...
          const opts = { gasPrice: generalStore.gasPrice };
          const method = crowdsaleContract.methods.setFinalizeAgent(
            finalizeAgentAddr
          );
    ...
    return () => {
      ...
      return attachToContract(abi, addr)
        .then(tokenContract => {
          ...
          const opts = { gasPrice: generalStore.gasPrice };
          const method = tokenContract.methods.setReleaseAgent(
            finalizeAgentAddr
          );
      ...
      return attachToContract(abi, addr)
        .then(tokenContract => {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            inPercentageUnit.push(
              obj.inPercentageUnit ? obj.inPercentageUnit : 0
            );
            inPercentageDecimals.push(
              obj.inPercentageDecimals ? obj.inPercentageDecimals : 0
            );
          }
          ...
          if (
            addrs.length === 0 &&
            inTokens.length === 0 &&
            inPercentageUnit.length === 0
          ) {
            if (inPercentageDecimals.length === 0) return Promise.resolve();
          }

          const opts = { gasPrice: generalStore.gasPrice };
          const method = tokenContract.methods.setReservedTokensListMultiple(
            addrs,
            inTokens,
            inPercentageUnit,
    ...
    () => {
      ...
      return attachToContract(abi, addr)
        .then(tokenContract => {
          ...
          const opts = { gasPrice: generalStore.gasPrice };
          const method = tokenContract.methods.transferOwnership(
            finalizeAgentAddr
          );
...
export const handlerForFile = (content, type) => {
  const checkIfTime =
    content.field === 'startTime' || content.field === 'endTime';
  let suffix = '';
...
export const handleContractsForFile = (
  content,
  index,
  contractStore,
  tierStore
...
const addSrcToFile = (content, index, contractStore, tierStore) => {
  ...
  if (isObservableArray(contractField) && field !== 'abi') {
    ...
    if (field !== 'src') {
      const body =
        field === 'abi' ? JSON.stringify(contractField) : contractField;
